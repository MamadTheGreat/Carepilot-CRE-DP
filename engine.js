...
function openSubitem(nodeId, subName, button) {
  ...
  const pages = model.pages?.length ? model.pages.map(renderPatientPage).join('') : '<p class="patient-muted">این مورد مستقیماً از بیمار پرسیده نمی‌شود و توسط موتور قواعد بالینی محاسبه می‌شود.</p>';
